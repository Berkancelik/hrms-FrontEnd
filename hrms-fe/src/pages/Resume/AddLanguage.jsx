...
export default function AddLanguage({ language }) {
  const AddLangugaedSchema = Yup.object().shape({
  ...
  const formik = useFormik({
    ...
    validationSchema: AddLangugaedSchema,
    onSubmit: (values) => {
      let resumeLanguageService = new LanguageService();

      resumeLanguageService.add(values).then(result=>console.log(result.data.data))
          swal("Başarılı!", "Dil bilgisi eklendi!", "success");
  ...
  return (
    <div>
      ...
      >
        <Modal.Header>Dil Bilgisi Ekleme</Modal.Header>
        <Modal.Description>
          <Container>
